...
%         u1=-2.5*rad.*(cos(teta*N)+sin(teta*N)).*(Z/Nz)/max(max(max(rad)))+(exp(-((X-Nx/2).^2+(Y-Ny/2).^2+(Z+5).^2)/50));
        ...
        u1=1.5*(exp(-((X-Nx/2).^2+(Y-Ny/2).^2+(Z-7).^2)/20));
...
%% energy density       
        wfi=0.5*Afi*mu.^2 + 0.5*sigma*gradfi + 0.5*AI*UI + 0.5*As*BI + 0.5*As*(fi.^2 -1).*US1 + 0.5*Af*(fi.^2).*BF;
...
%% fi
        ...
        F=Afi*((3*fi.^2-1-2*ep1*fi.*Co).*mu-ep*lapmu)+2*As*fi.*(fi.^2-1).*US1+Af*fi.*BF-0.5*AI*lapUI-sigma*lapfi;
...
%% Stress tensor
	...
	-0.5*AI*gam1*gu1(:,:,:,1).*gfi(:,:,:,1)-Afi*ep*gmu(:,:,:,1).*gfi(:,:,:,1)+Afi*ep*mu.*ggfi1(:,:,:,1));
    str(:,:,:,2,2)=(wfi(:,:,:) -fi(:,:,:).*F(:,:,:)-sigma*gfi(:,:,:,2).*gfi(:,:,:,2)...
	-0.5*AI*gam1*gu1(:,:,:,2).*gfi(:,:,:,2)-Afi*ep*gmu(:,:,:,2).*gfi(:,:,:,2)+Afi*ep*mu.*ggfi2(:,:,:,2));
    str(:,:,:,3,3)=(wfi(:,:,:) -fi(:,:,:).*F(:,:,:)-sigma*gfi(:,:,:,3).*gfi(:,:,:,3)...
	-0.5*AI*gam1*gu1(:,:,:,3).*gfi(:,:,:,3)-Afi*ep*gmu(:,:,:,3).*gfi(:,:,:,3)+Afi*ep*mu.*ggfi3(:,:,:,3));
 
	str(:,:,:,1,2)=(-sigma*gfi(:,:,:,1).*gfi(:,:,:,2)...
	-0.5*AI*gam1*gu1(:,:,:,2).*gfi(:,:,:,1)-Afi*ep*gmu(:,:,:,2).*gfi(:,:,:,1)+Afi*ep*mu.*ggfi2(:,:,:,1));
	str(:,:,:,1,3)=(-sigma*gfi(:,:,:,1).*gfi(:,:,:,3)...
	-0.5*AI*gam1*gu1(:,:,:,3).*gfi(:,:,:,1)-Afi*ep*gmu(:,:,:,3).*gfi(:,:,:,1)+Afi*ep*mu.*ggfi3(:,:,:,1));
	str(:,:,:,2,1)=(-sigma*gfi(:,:,:,2).*gfi(:,:,:,1)...
	-0.5*AI*gam1*gu1(:,:,:,1).*gfi(:,:,:,2)-Afi*ep*gmu(:,:,:,1).*gfi(:,:,:,2)+Afi*ep*mu.*ggfi1(:,:,:,2));
	str(:,:,:,2,3)=(-sigma*gfi(:,:,:,2).*gfi(:,:,:,3)...
	-0.5*AI*gam1*gu1(:,:,:,3).*gfi(:,:,:,2)-Afi*ep*gmu(:,:,:,3).*gfi(:,:,:,2)+Afi*ep*mu.*ggfi3(:,:,:,2));
	str(:,:,:,3,1)=(-sigma*gfi(:,:,:,3).*gfi(:,:,:,1)...
	-0.5*AI*gam1*gu1(:,:,:,1).*gfi(:,:,:,3)-Afi*ep*gmu(:,:,:,1).*gfi(:,:,:,3)+Afi*ep*mu.*ggfi1(:,:,:,3));
	str(:,:,:,3,2)=(-sigma*gfi(:,:,:,3).*gfi(:,:,:,2)...
	-0.5*AI*gam1*gu1(:,:,:,2).*gfi(:,:,:,3)-Afi*ep*gmu(:,:,:,2).*gfi(:,:,:,3)+Afi*ep*mu.*ggfi2(:,:,:,3)); 
