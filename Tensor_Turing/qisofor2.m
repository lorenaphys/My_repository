...
%         potfu=2*(((fi).^2-1).^2).*(((beta*u-R1)))+2*(fi.^2).*(beta*u-0);
        ...
        If=40*sum(sum(sum(G.*(fi>=-.99))))/Nx/Ny/Nz;
        %If(fi<=0)=0; 
...
%	fi(:,:,1)=0; 
 ...
 %       fi(:,:,1)=fiini(:,:,1);
    ...
    %%
    str1o=Afi*mu.^2+As*(((fi).^2-1).^2).*((u-0).^2).*((u-1).^2)+Af*fi.*fi.*((u-0).^2)+gamma*gradient(fi).*gradient(u)+ 0.5*sigma*gradient(fi).*gradient(fi)+0.5*gradient(u).*gradient(u)-fi.*F;
    str1=zeros(Nx,Ny,Nz);
    for hdo=1:Nx
    str1=str1o(hdo,hdo,hdo);
    end
    
    str2=-sigma*gradient(fi).*gradient(fi)-gamma*gradient(u).*gradient(fi);
    str3=-2*ep*gradient(mu).*gradient(fi);
    str4=+2*ep*mu.*gradient(gradient(fi));
